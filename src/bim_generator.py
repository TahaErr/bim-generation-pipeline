...
class SD15Backend:
    def __init__(self, condition_mode="depth_v2", seed=42):
        ...
        print(f"[SD1.5] ControlNet: {cn_id}")
        ...
        print(f"[SD1.5] Ready | {self.device}")
...
class SDXLBackend:
    def __init__(self, condition_mode="depth_v2", seed=42):
        ...
        print(f"[SDXL] Ready | {self.device}")

    def generate(self, condition_map, prompt, negative_prompt,
                 steps=30, guidance=10.0, cn_scale=0.8, size=1024):
        cond = condition_map.resize((size, size))
        with torch.inference_mode():
            out = self.pipe(prompt=prompt, negative_prompt=negative_prompt, image=cond,
                           num_inference_steps=steps, guidance_scale=guidance,
...
class FluxBackend:
    def __init__(self, seed=42):
        ...
        self.pipe.enable_model_cpu_offload()
        print(f"[Flux] Ready | {self.device}")

    def generate(self, condition_map, prompt, negative_prompt="",
                 steps=24, guidance=3.5, cn_scale=0.5, size=1024):
        cond = condition_map.resize((size, size))
        with torch.inference_mode():
